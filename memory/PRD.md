...
- **Production audit + slim requirements + fail-fast guards (Ubuntu 24.04)**: (a) AST-scanned every backend .py file — 10 packages were in requirements.txt but never imported (boto3, jq, numpy, pandas, passlib, python-jose, requests, requests-oauthlib, typer, email-validator, cryptography) — all removed. Runtime `requirements.txt` shrank 30 → 13 packages, install ~90s → ~15s, no native-build packages (jq/numpy/pandas/cryptography all gone). (b) Split dev tooling into new `backend/requirements-dev.txt` (black/flake8/isort/mypy/pytest). (c) Added `ENVIRONMENT=production` fail-fast in server.py — raises RuntimeError at import time if `JWT_SECRET==dev-secret` or bootstrap admin/mod password still equals the shipped defaults; verified working. (d) Fixed the `CORS_ORIGINS=* + allow_credentials=True` browser-quirk — now auto-disables credentials when origins is wildcard (safe because we auth via Authorization header, not cookies). (e) New `deploy/PRODUCTION_AUDIT.md` — 30-category audit report with dependency matrix, confirmed/potential/safe findings, and unchanged deploy commands. (f) testing_agent iteration 3: **12/12 backend regression tests pass** — listings, categories, JWT auth, profile, Steam inventory (220 real items), server-authoritative listings, order reserve state machine, admin login (bcrypt intact), admin stats, Stripe webhook endpoint, CORS preflight.
